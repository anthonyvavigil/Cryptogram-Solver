...
public class CrytographSolver {
			...
			if (nextLine.toLowerCase().equals("q")) {
				...
				if (doRelFreq) {
					...
					while (scn2.hasNextLine()) {
						...
						if (cou % 500 == 0) {
							System.out.println("running, checked " + cou + " responses");
						}
						cou++;
						// sol.add(relativeFrequencySolve(nextLine.toLowerCase(), tolerance,
						// letterFreq));
					}
	...
	public static String relativeFrequencySolve(String input, int tol, String letterFrequencies) {
		...
		// loops through every character in the cipher, if it already has a letter
		// assigned to it, add one to appearances, if not, create a letter
		for (int i = 0; i < tot; i++) {
		...
		for (int h = 0; h < freqList.size(); h++) {
			freqList.get(h).setCorrespondent(String.valueOf(letterFrequencies.charAt(h)));
			freqList.get(h).setCorrespondent(letterFrequencies.charAt(h));
			if (printedFreq == false) {
				System.out.println(freqList.get(h).getNameString() + "||" + freqList.get(h).getFreq() + "||"
						+ freqList.get(h).getCorrespondentString());
			}
		}
		printedFreq = true;

		// changes each letter in the input to correspond with its most likely
		// correspondent
		String decrypted = "";
	...
	public static ArrayList<String> methodicSolve(String input, ArrayList<String> dic, int tol) {
		...
		// initialize letter possibilities - one letter for each of the alphabet, one
		// letter correspondence for each letter of the alphabet
		...
		while (!methodFinished) { // loops through words in list
			...
			if (curWord.length() == 1) {
				...
				allLetters.get(((int) curWord.charAt(0) - 97)).setAorI();
			}
			/*
			...
			while (dic.get(w).length() <= inputArr.get(count).length() && n < dic.size() && dic.get(w).length() <= 2) { // loops
																														// //
																														// array
				if (dic.get(w).length() == inputArr.get(count).length()) {
					...
					for (int j = 0; j < tempDic.length(); j++) { // loops through word from dictionary
						if (tempDic.length() - j >= 2) { // if there are two more letters to look at
							...
							char char1 = tempInp.charAt(j); // gets pair of characters to be analyzed - (characters so
															// they can easily be switched to index)
							...
							for (int t = 0; t < l1.getCorrespondences().size(); t++) {
								for (int y = 0; y < l2.getCorrespondences().size(); y++) {
									...
									if (tempA.equals(tempB)) {
										...
										if (!allLetters.get(((int) char1) - 97).isAorI()) {
											allLetters.get(((int) char1) - 97)
													.addTempPossibility(tempDic.substring(j, j + 1));
										}
										if (!allLetters.get(((int) char2) - 97).isAorI()) {
											allLetters.get(((int) char2) - 97)
													.addTempPossibility(tempDic.substring(j + 1, j + 2));
										}
									} else {
			...
			if (count < inputArr.size() - 1) { // keeps looping through the input by word
				...
				for (int i = 0; i < allLetters.size(); i++) {
					...
					System.out.println(allLetters.get(i).getName() + ":corresp size:"
							+ allLetters.get(i).getCorrespondences().size());
					System.out.println("end of line");
				}
				System.out.println("dic: " + dic.size());
				System.out.println("inpArr: " + inputArr.size() + ":" + count);
				methodFinished = transitionMethods();
			}

		}
		bruteForce(input, allLetters, dic);
		return dic;
	}

	public static ArrayList<String> bruteForce(String input, ArrayList<RichLetter> allLetters, ArrayList<String> dic) {

		return null;
	}

	public static boolean transitionMethods() {
		Scanner scn = null;
		try {
			scn = new Scanner(System.in);
		} catch (Exception e) {
		...
		while (unpaused == false) {
			System.out.println(
					"narrowed down list, would you like to perform a brute force solve? (Y for yes, N for no)");
			String a = scn.nextLine();
	...
	public static void pause() {
		...
		while (unpaused == false) {
			System.out.println("operation paused, press P to continue");
			String a = scn.nextLine();
			if (a.toUpperCase().equals("P"))
				unpaused = true;
		}
	}
